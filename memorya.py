...
def control_hardware():
    shell=Tk()
    shell.geometry('500x500+100+100')
    shell.title("Control de Hardware")
    shell.mainloop
...
mnuArchivo.add_command(label="Pasar proceso a memoria virtual", command=control_hardware)
mnuArchivo.add_command(label="Regresar proceso a memoria principal.", command=control_hardware)
...
#Inicio de estructura de la memoria Ram
...
def eliminar_P(op):
	...
	if(ind != -1):
		...
		for i in range(noProcesos):
			posicion.insert(i,copy.deepcopy(int(i)))
			print(posicion[i])
			tablapros.insert("insert",str(proc[i].id)+"\t     "+str(proc[i].size)+"\n")
			ver[i+1]= Button(ventana, text="ver"+str(int(proc[i].id)),command=lambda:proc[copy.deepcopy(i)].showInfo())
